"""
Script pour générer des commandes fictives.
Exécuter avec: docker compose exec backend python -m src.db.seed_fake_orders
"""

import random
from datetime import datetime, date, time, timedelta, timezone
from sqlalchemy.orm import Session
from src.db.session import get_db
from src.users.models import User
from src.menu.models import MenuItem

def seed_fake_orders(count: int = 10):
    db = next(get_db())
    
    # Récupérer les items du menu par type
    menus = db.query(MenuItem).filter(MenuItem.item_type == "menu").all()
    boissons = db.query(MenuItem).filter(MenuItem.item_type == "boisson").all()
    bonus = db.query(MenuItem).filter(MenuItem.item_type == "upsell").all() # Mapping upsell -> bonus_id
    
    if not menus:
        print("❌ Aucun menu trouvé. Veuillez d'abord initialiser le menu avec init_menu.py")
        return

    first_names = ["Léo", "Manon", "Hugo", "Emma", "Nathan", "Léa", "Enzo", "Chloé", "Louis", "Camille"]
    last_names = ["Martin", "Bernard", "Thomas", "Petit", "Robert", "Richard", "Durand", "Dubois", "Moreau", "Laurent"]
    
    today = date.today()
    slots = [time(12, 0), time(12, 30), time(13, 0), time(13, 30), time(19, 0), time(19, 30), time(20, 0), time(20, 30)]

    print(f"🔄 Génération de {count} commandes fictives...")

    for i in range(1, count + 1):
        email = f"dev_test_{i}@example.com"
        
        # Vérifier si l'utilisateur existe déjà
        user = db.query(User).filter(User.email == email).first()
        if not user:
            user = User(email=email)
            db.add(user)
        
        user.prenom = random.choice(first_names)
        user.nom = random.choice(last_names)
        user.email_verified = True
        user.is_cotisant = random.choice([True, False])
        user.date_reservation = today + timedelta(days=random.randint(0, 2))
        user.heure_reservation = random.choice(slots)
        
        # Choix aléatoire des items
        user.menu_id = random.choice(menus).id
        user.boisson_id = random.choice(boissons).id if boissons and random.random() > 0.2 else None
        user.bonus_id = random.choice(bonus).id if bonus and random.random() > 0.5 else None
        
        user.habite_residence = random.choice([True, False])
        if user.habite_residence:
            user.numero_immeuble = f"U{random.randint(1, 7)}"
            user.numero_chambre = random.randint(100, 500)
        else:
            user.adresse = f"{random.randint(1, 100)} Rue de l'Exemple, 91000 Évry"
            
        user.phone = f"06{random.randint(10, 99)}{random.randint(10, 99)}{random.randint(10, 99)}{random.randint(10, 99)}"
        user.payment_status = "completed"
        user.status = "confirmed"
        user.last_ip = f"192.168.1.{random.randint(1, 254)}"
        
        user.total_amount = 1.0 # Prix fixe pour le test
        user.payment_date = datetime.now(timezone.utc)
        
    try:
        db.commit()
        print(f"✅ {count} commandes fictives créées avec succès !")
    except Exception as e:
        db.rollback()
        print(f"❌ Erreur lors du seeding: {e}")

if __name__ == "__main__":
    import sys
    count = 10
    if len(sys.argv) > 1:
        try:
            count = int(sys.argv[1])
        except ValueError:
            pass
    seed_fake_orders(count)
